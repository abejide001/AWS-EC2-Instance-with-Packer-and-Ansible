...
- Click on the Add rule, and select the `type - custom TCP rule`, add the `port range`, **ensure you add the appplication port**, `port - 5000` so you can access it via the browser, also choose the source to be `custom` and `0.0.0.0/0`(allow all traffic)
